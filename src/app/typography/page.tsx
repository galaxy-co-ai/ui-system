...
const textStyles = [
  ...
  { label: "Small", size: "13px", lineHeight: "1.5", font: "body", color: "white/50" },
  { label: "Eyebrow", size: "11px", lineHeight: "1", font: "code", color: "white/30", extra: "uppercase, tracking: 0.2em" },
  { label: "Code", size: "12px", lineHeight: "1.5", font: "code", color: "white/70" },
  { label: "Caption", size: "10px", lineHeight: "1.4", font: "code", color: "white/25" },
];
  ...
  return (
    <section className="mx-auto max-w-3xl px-8 pb-32 pt-20">
          ...
          >
            <div className="flex items-start justify-between gap-4 mb-4">
              <div>
                ...
                <span
                  className="text-[11px] text-white/30 mt-0.5 block"
                  style={{ fontFamily: "var(--font-code)" }}
                >
                  {variable}
                </span>
              </div>
              <span className="text-[11px] text-white/20 shrink-0">{usage}</span>
            </div>
            {/* Specimen */}
            <div
              className={`${className} text-[28px] text-white/80 leading-tight mt-2`}
            >
              {specimen}
            </div>
            {/* Alphabet */}
            <div
              className={`${className} text-[16px] text-white/25 mt-4 leading-relaxed`}
            >
              AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz
            </div>
            <div
              className={`${className} text-[16px] text-white/25 mt-1`}
            >
              0123456789 !@#$%^&*()
            </div>
            {/* Fallback stack */}
            <span
              className="block mt-4 text-[10px] text-white/20"
              style={{ fontFamily: "var(--font-code)" }}
            >
              Fallback: {fallback}
            </span>
      ...
      <div className="mt-16">
        ...
        <div className="space-y-6">
          {headingSizes.map(({ label, css, font, tracking, weight }) => (
            <div key={label} className="border-b border-white/[0.04] pb-6 last:border-0">
              <div className="flex items-baseline gap-3 mb-2">
                <span
                  className="text-[11px] text-white/25 w-20 shrink-0"
                  style={{ fontFamily: "var(--font-code)" }}
                >
                  {label}
                </span>
              ...
              <div
                ...
                className={`text-white ${css.includes("clamp") ? "" : ""}`}
              >
                {css.includes("clamp") ? (
                  <span className={
        ...
        <div className="space-y-2">
          {textStyles.map(({ label, size, lineHeight, font, extra }) => (
            <div
              ...
              style={{ borderRadius: "var(--radius-lg)" }}
            >
              <span className="text-[12px] text-white/50 w-20 shrink-0">{label}</span>
              <span
              ...
              <span
                className="text-[10px] text-white/20 shrink-0 text-right"
                style={{ fontFamily: "var(--font-code)" }}
              >
                {size} / {lineHeight}
      ...
      <div className="mt-16">
        ...
        <div className="space-y-3 text-[13px] text-white/40 leading-relaxed">
          <p>
            <span className="text-white/70 font-medium">Use clamp() for headings.</span>{" "}
          ...
          <p>
            <span className="text-white/70 font-medium">65ch max width for body text.</span>{" "}
            Long-form content should never exceed 65 characters per line.
            Use max-w-[55ch] to max-w-[65ch] depending on context.
          </p>
          <p>
            <span className="text-white/70 font-medium">tabular-nums on changing data.</span>{" "}
          ...
          <p>
            <span className="text-white/70 font-medium">Code font for tokens and variables.</span>{" "}
